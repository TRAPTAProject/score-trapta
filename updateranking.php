<?php
	...
	if ($row = mysqli_fetch_array($result)) {
		...
		if ($row['password']==$password) {
			...
			$file1 = fopen($username."/ranking.html","w");
			// header
			...
			foreach ($categArray as $categObj) {
				...
				$cut = $categObj->{'cut'};
				fwrite($file1,'<h1>'.$categTitle.'</h1><h4>Série '.$runIndex.' - Volée '.$volleyIndex.'</h4>');
				...
				foreach ($archerArray as $archerItem) {
					...
					if ($shift==0) {
						...
						fwrite($file1, '<td><img src="http://score.trapta.eu/images/arrowdown.png">'.$shift.'</td>');
					}
					else if ($shift>0) {
						fwrite($file1, '<td><img src="http://score.trapta.eu/images/arrowup.png">+'.$shift.'</td>');
					}
					// trend
					if ($trend==-2) fwrite($file1, '<td><img src="http://score.trapta.eu/images/smiley-2.png"></td>');
					else if ($trend==-1) fwrite($file1, '<td><img src="http://score.trapta.eu/images/smiley-1.png"></td>');
					else if ($trend==1) fwrite($file1, '<td><img src="http://score.trapta.eu/images/smiley1.png"></td>');
					else if ($trend==2) fwrite($file1, '<td><img src="http://score.trapta.eu/images/smiley2.png"></td>');
			...
			foreach ($categArray as $categObj) {
				...
				foreach ($archerArray as $archerItem) {
					...
					$file3 = fopen($username."/scorecards/scorecard".$id.".html","w");	
					$counter = 1;
			...
			$file2 = fopen($username."/teams.html","w");
			if (isset($obj->{'team'})) {				
				// header
